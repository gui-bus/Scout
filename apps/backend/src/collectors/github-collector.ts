...
export class GithubCollector extends BaseCollector {
  ...
  async collect(): Promise<any[]> {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch GitHub issues from ${this.repo}: ${response.statusText}`);
    }
  ...
  private detectModality(issue: any): string | null {
    ...
    if (text.includes("remoto") || text.includes("remote") || text.includes("teletrabalho")) {
      return "Remoto";
    }
    if (text.includes("hibrido") || text.includes("híbrido") || text.includes("hybrid")) {
      return "Híbrido";
    }
  ...
  private detectLevel(issue: any): string | null {
    ...
    if (labels.includes("júnior") || labels.includes("junior") || title.includes("junior") || title.includes("júnior") || title.includes("jr")) {
      return "Júnior";
    }
    if (labels.includes("pleno") || title.includes("pleno") || title.includes("mid")) {
      return "Pleno";
    }
    if (labels.includes("sênior") || labels.includes("senior") || title.includes("senior") || title.includes("sênior") || title.includes("sr")) {
      return "Sênior";
    }
    if (labels.includes("estágio") || labels.includes("estagio") || labels.includes("intern") || title.includes("estag")) {
      return "Estágio";
    }
    return null;
  }

  private parseTitleAndCompany(rawTitle: string): { title: string; company: string | null } {
  ...
  private extractTechnologies(issue: any): string | null {
    const list: string[] = [];
    const ignoreWords = ["remoto", "hibrido", "híbrido", "presencial", "junior", "júnior", "pleno", "senior", "sênior", "estágio", "estagio", "vaga", "clt", "pj"];
  ...
  private detectLocation(issue: any, modality: string | null): string | null {
    ...
    if (brackets) {
      for (const bracket of brackets) {
        ...
        if (val.toLowerCase() !== "remoto" && val.toLowerCase() !== "hibrido" && val.toLowerCase() !== "híbrido" && val.toLowerCase() !== "presencial") {
          return val;
        }
      }
    }
