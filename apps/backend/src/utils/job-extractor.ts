...
export function extractTechnologiesFromText(description: string | null | undefined): string | null {
...
export function extractMetadata(description: string | null | undefined): ExtractedMetadata {
  ...
  const isPj = /\b(pj|prestador\s+de\s+servico|cooperado|pessoa\s+juridica)\b/i.test(descLower);
  ...
  const formsRegex = /(https?:\/\/(?:[a-zA-Z0-9-]+\.)*(?:google\.com\/forms|forms\.gle|typeform\.com|forms\.office\.com|gupy\.io)\/[^\s\)]+)/gi;
  let formMatch;
